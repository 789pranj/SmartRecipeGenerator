...
import { LogOut, UserPlus, LogIn, Home, Plus, Album, Save, Menu } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-3 bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-600 shadow-xl sticky top-0 z-50 text-white">
        ...
        {!user ? (
          ...
          <div className="flex items-center space-x-3 justify-evenly">
            ...
            <button
              ...
              className="flex items-center space-x-1 px-4 py-2 rounded-lg bg-white text-red-600 hover:bg-red-200 hover:text-red-700 transition-all duration-200 shadow-md"
            >
      ...
      <button
        className="md:hidden p-2 rounded-lg bg-white text-purple-700 hover:bg-purple-100 hover:text-purple-900 transition-all duration-200 shadow-md"
      ...
      {open && (
        <div className="absolute top-16 right-4 w-56 bg-white text-purple-700 rounded-lg shadow-lg p-4 flex flex-col space-y-3 md:hidden z-50">
          {!user ? (
            <>
              <Link to="/" onClick={() => setOpen(false)} className="flex items-center space-x-2 hover:text-purple-900">
                <UserPlus className="w-5 h-5" />
                <span>Signup</span>
              </Link>
              <Link to="/login" onClick={() => setOpen(false)} className="flex items-center space-x-2 hover:text-purple-900">
                <LogIn className="w-5 h-5" />
                <span>Login</span>
              </Link>
            </>
            ...
            <>
              <Link to="/saved" onClick={() => setOpen(false)} className="flex items-center space-x-2 hover:text-orange-900">
                <Save className="w-5 h-5" />
                <span>Saved</span>
              </Link>
              <Link to="/" onClick={() => setOpen(false)} className="flex items-center space-x-2 hover:text-green-900">
                <Album className="w-5 h-5" />
                <span>ALL</span>
              </Link>
              <Link to="/create-recipe" onClick={() => setOpen(false)} className="flex items-center space-x-2 hover:text-yellow-900">
                <Plus className="w-5 h-5" />
                <span>Create</span>
              </Link>
              <button
